...
### Fixed
- **Test timeouts configured in `test-config.yaml` now bound the HTTP request, not just the jest test.** `test_settings.timeouts` was read by `configHelpers.getTimeout()` and handed to jest, while the request underneath was governed by a different function entirely — `getTimeout()` in `@mcp-abap-adt/adt-clients`, which reads `SAP_TIMEOUT_*` from the environment and nothing else. Nothing wrote those variables, so a config saying 120s produced a request with the library's built-in 45s. On a live system that aborted `activateObjectsGroup` mid-flight for a group of 24 objects; the fallback path then left every object inactive, and teardown failed on 15 of them because ADT will not delete an inactive object that is still referenced. `default`, `long`, `csrf` and `critical` are now published to `SAP_TIMEOUT_DEFAULT` / `_LONG` / `_CSRF` / `_CRITICAL` when the config loads and again in `globalSetup`, so workers forked afterwards inherit them. An explicitly exported variable still wins — a value set for one run is meant, and a config file should not overrule it. The bridge lives in a dependency-free `helpers/timeoutEnv.ts` because `globalSetup` is resolved outside the transform used for tests and cannot pull in the helper chain. Nothing changes for the shipped server, which keeps the library defaults. ([#174](https://github.com/fr0ster/mcp-abap-adt/issues/174))
