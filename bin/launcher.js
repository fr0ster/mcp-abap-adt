...
/**
 * Launcher for MCP ABAP ADT Server
 *
 ...
 *
 * Usage:
 *   node bin/launcher.js [options]
 *   node bin/launcher.js --env=e19.env --transport=stdio
 *   node bin/launcher.js --env=/path/to/.env --transport=http
 */
...
function loadEnvFile(envFilePath) {
  ...
  try {
    ...
    if (result.error) {
      ...
      process.stderr.write(`[LAUNCHER] ✗ Failed to load .env file: ${envFilePath}\n`);
      process.stderr.write(`[LAUNCHER] Error: ${result.error.message}\n`);
      return false;
    }

    // Only write to stderr if not in stdio mode (stdio mode requires clean JSON only)
    if (!isStdio) {
      process.stderr.write(`[LAUNCHER] ✓ Loaded .env file: ${envFilePath}\n`);
    }
    return true;
  } catch (error) {
    // Always write to stderr (safe even in stdio mode)
    process.stderr.write(`[LAUNCHER] ✗ Failed to load .env file: ${envFilePath}\n`);
    process.stderr.write(`[LAUNCHER] Error: ${error.message}\n`);
    return false;
  }
}
...
function showHelp() {
  ...
  console.log(`
MCP ABAP ADT Server Launcher v${pkg.version}
...
Examples:
  node bin/launcher.js                                    # Use default .env
  node bin/launcher.js --env=e19.env                      # Use specific .env file
  node bin/launcher.js --env=e19.env --transport=stdio    # Use .env and stdio transport
  node bin/launcher.js --transport=http                   # HTTP mode (no .env required)
...
function main() {
  ...
  if (args.envFile || isEnvRequired) {
    ...
    if (!envLoaded && isEnvRequired) {
      // Always write to stderr (safe even in stdio mode)
      process.stderr.write(`[LAUNCHER] ✗ ERROR: .env file is required for transport mode '${args.transport || 'stdio'}'\n`);
      process.stderr.write(`[LAUNCHER]   Use --env=/path/to/.env to specify custom location\n`);
      process.exit(1);
    } else if (!envLoaded && args.envFile) {
      // Always write to stderr (safe even in stdio mode)
      process.stderr.write(`[LAUNCHER] ✗ ERROR: .env file not found: ${args.envFile}\n`);
      process.exit(1);
    }
  } else {
  ...
  try {
    ...
    // Always write to stderr (safe even in stdio mode)
    process.stderr.write(`[LAUNCHER] ✗ Failed to start server: ${error.message}\n`);
    if (error.stack) {
