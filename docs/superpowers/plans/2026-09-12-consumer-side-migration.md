...
- **`withLock` is for handlers that own the whole lifetime of a lock — the `PUT`-shaped updates, and nothing else.** The corpus says what locks: an update is lock/PUT/unlock, while a create, an activation and a deletion are bare POSTs with no lock. And the fifteen `low`-tier `LockX` tools hand the handle back on purpose, so releasing before returning would destroy them; they are never wrapped.
...
The utils-backed ones (`handleGetObjectStructure`, `handleGetAllTypes`, `handleGetInactiveObjects`, `handleGetSqlQuery`, `handleGetTableContents`) take `client.getUtils(ourUtils)` — and pass **no** `analyse`, because none of those five members declares `<E extends IAdtError>` in 19. `search` is the one `AdtUtils` member that does, and `handleSearchObject` passes one. Their result strategy is ours; their verdict is not. Do not add an argument the signature does not have to satisfy a rule; the rule is written by signature for exactly this reason.
...
Where the handler holds a lock — every `high` update, and only those — the
read-patch-write goes inside `withLock`, not beside it. A `low`-tier update takes
the handle as an argument and must not acquire or release one. `sequence` is right only for the part that can
...
**Files:**
- Create: `src/__tests__/unit/handlerInvariants.test.ts`, `src/__tests__/unit/legacyContract.test.ts`
...
it('every client call that accepts an analyse is given one', () => {
  ...
  for (const file of handlers) {
    ...
    // contract; the `Legacy` one is pinned by the test below, because
    // `createAdtClient` is declared to return `AdtClient` and the checker never
    // sees the `Legacy` declarations from a call site.
...
`SAP_SYSTEM_TYPE=legacy` is a supported deployment running a subset of the tools, and those tools run **the same handler files** this work rewrites. `available_in` does not help: it hides the tools that cannot run on legacy at all, not the ones that can. Four of the ten overridden `Legacy` classes take no strategy on some members; twenty-three legacy-declaring tools reach them.
...
- [ ] **Step 3: For those twenty-three handlers, prefer a member that accepts a strategy**
...
Go through that list once. Where the handler can reach the same result through a member the `Legacy` class does parameterise, use it. What is left is the measured cost of two contracts, and it belongs in the PR description rather than in a comment nobody reads.
...
```bash
git add src/__tests__/unit/handlerInvariants.test.ts src/__tests__/unit/legacyContract.test.ts
git commit -m "test(handlers): the invariants, and what the legacy contract decides alone"
```
