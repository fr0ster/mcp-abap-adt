...
describe('Function Low-Level Handlers Integration (FUGR + FM)', () => {
  ...
  describe('Full Workflow (FUGR + FM)', () => {
    it('should execute full workflow: FUGR (V→C→L→U→A) then FM (V→C→L→U→U→A)', async () => {
      ...
      const functionGroupDescription = testCase.params.function_group_description || functionGroupName;
      const functionModuleDescription = testCase.params.function_module_description || functionModuleName;
      ...
      const diagnosticsTracker = createDiagnosticsTracker('function_low_full_workflow', testCase, session, {
      ...
      try {
        ...
        testLogger?.info(`📦 [FUGR] Starting function group workflow: ${functionGroupName}`);

        // Step 1.1: Validate FUGR
        debugLog('FUGR-VALIDATE', `Validating ${functionGroupName}`);
        const validateFGResponse = await handleValidateFunctionGroup({connection, logger: testLogger}, {
        ...
        if (validateFGResponse.isError) {
          const errorMsg = validateFGResponse.content[0]?.text || 'Unknown error';
          if (errorMsg.includes('already exists')) {
            testLogger?.info(`⏭️  Function group ${functionGroupName} already exists, skipping test`);
            return;
          }
        ...
        const lockFGResponse = await handleLockFunctionGroup({connection, logger: testLogger}, {
          function_group_name: functionGroupName,
          session_id: session.session_id,
          session_state: session.session_state
        });
        ...
        const unlockFGResponse = await handleUnlockFunctionGroup({connection, logger: testLogger}, {
          function_group_name: functionGroupName,
        ...
        if (unlockFGResponse.isError) {
          const errorMsg = unlockFGResponse.content[0]?.text || 'Unknown error';
        ...
        const activateFGResponse = await handleActivateFunctionGroup({connection, logger: testLogger}, {
          function_group_name: functionGroupName,
          session_id: session.session_id,
          session_state: session.session_state
        });
        ...
        if (activateFGResponse.isError) {
          const errorMsg = activateFGResponse.content[0]?.text || 'Unknown error';
        ...
        testLogger?.info(`✅ [FUGR] Function group workflow completed: ${functionGroupName}`);
        await delay(getOperationDelay('activate', testCase));

        // ==================== PART 2: Function Module ====================
        testLogger?.info(`📦 [FM] Starting function module workflow: ${functionModuleName}`);

        // Step 2.1: Validate FM
        debugLog('FM-VALIDATE', `Validating ${functionModuleName}`);
        const validateFMResponse = await handleValidateFunctionModule({connection, logger: testLogger}, {
        ...
        if (validateFMResponse.isError) {
          const errorMsg = validateFMResponse.content[0]?.text || 'Unknown error';
          ...
          if (errorMsg.includes('already exists')) {
            testLogger?.info(`⏭️  Function module ${functionModuleName} already exists, skipping test`);
            return;
          }
        ...
        const lockFMResponse = await handleLockFunctionModule({connection, logger: testLogger}, {
          function_module_name: functionModuleName,
          function_group_name: functionGroupName,
          session_id: session.session_id,
          session_state: session.session_state
        ...
        debugLog('FM-UPDATE', `Updating ${functionModuleName} with source code`);
        ...
        if (updateFMResponse.isError) {
          const errorMsg = updateFMResponse.content[0]?.text || 'Unknown error';
        ...
        const unlockFMResponse = await handleUnlockFunctionModule({connection, logger: testLogger}, {
          function_module_name: functionModuleName,
          function_group_name: functionGroupName,
          lock_handle: fmLockHandle,
          session_id: fmLockSession.session_id!,
        ...
        if (unlockFMResponse.isError) {
          const errorMsg = unlockFMResponse.content[0]?.text || 'Unknown error';
        ...
        const activateFMResponse = await handleActivateFunctionModule({connection, logger: testLogger}, {
          function_module_name: functionModuleName,
          function_group_name: functionGroupName,
          session_id: session.session_id,
          session_state: session.session_state
        ...
        if (activateFMResponse.isError) {
          const errorMsg = activateFMResponse.content[0]?.text || 'Unknown error';
        ...
        testLogger?.info(`✅ [FM] Function module workflow completed: ${functionModuleName}`);
        testLogger?.info(`✅ ✅ Full workflow completed successfully`);

      } catch (error: any) {
        const errorMessage = error.message || String(error);
        let failedStep = 'UNKNOWN';
        if (errorMessage.includes('FUGR validation')) failedStep = 'FUGR-VALIDATE';
        else if (errorMessage.includes('FUGR create')) failedStep = 'FUGR-CREATE';
        else if (errorMessage.includes('FUGR lock')) failedStep = 'FUGR-LOCK';
        else if (errorMessage.includes('FUGR unlock')) failedStep = 'FUGR-UNLOCK';
        else if (errorMessage.includes('FUGR activate')) failedStep = 'FUGR-ACTIVATE';
        else if (errorMessage.includes('FM validation')) failedStep = 'FM-VALIDATE';
        ...
        else if (errorMessage.includes('FM activate')) failedStep = 'FM-ACTIVATE';
        ...
        if (shouldCleanup && session) {
          ...
          if (fmLockHandle && fmLockSession) {
            try {
              await handleUnlockFunctionModule({connection, logger: testLogger}, {
                function_module_name: functionModuleName,
                function_group_name: functionGroupName,
                lock_handle: fmLockHandle,
                session_id: fmLockSession.session_id!,
          ...
          try {
            ...
            testLogger?.warn(`⚠️  Failed to cleanup function module ${functionModuleName}: ${cleanupError.message || cleanupError}`);
          }
          ...
          try {
            ...
            testLogger?.warn(`⚠️  Failed to cleanup function group ${functionGroupName}: ${cleanupError.message || cleanupError}`);
          }
        } else {
          testLogger?.info(`⚠️ Cleanup skipped (cleanup_after=false) - objects left for analysis`);
        }
