...
ENDCLASS.`;
        ...
        debugLog(
          'TEST_START',
          `Starting unit test workflow for class: ${containerClass}`,
          {
            containerClass,
            testClass,
        ...
        try {
          ...
          debugLog(
            'LOCK_TEST_CLASSES',
            `Starting lock for test classes of ${containerClass}`,
            {
          ...
          if (lockResponse.isError) {
            throw new Error(
              `Lock test classes failed: ${lockResponse.content[0]?.text || 'Unknown error'}`,
            );
          }
          ...
          diagnosticsTracker.persistLock(
            testClassesLockSession,
            testClassesLockHandle,
            {
              object_type: 'CLAS',
              object_name: containerClass,
            },
          ...
          debugLog(
            'UPDATE_TEST_CLASSES',
            `Starting update for test classes of ${containerClass}`,
            {
              lock_handle: testClassesLockHandle,
          ...
          if (updateResponse.isError) {
            throw new Error(
              `Update test classes failed: ${updateResponse.content[0]?.text || 'Unknown error'}`,
            );
          }
          ...
          debugLog(
            'UNLOCK_TEST_CLASSES',
            `Starting unlock for test classes of ${containerClass}`,
            {
              lock_handle: testClassesLockHandle,
              session_id: testClassesLockSession.session_id,
            },
          ...
          if (unlockResponse.isError) {
            throw new Error(
              `Unlock test classes failed: ${unlockResponse.content[0]?.text || 'Unknown error'}`,
            );
          }
          ...
          debugLog(
            'UNLOCK_TEST_CLASSES',
            'Test classes unlocked successfully',
            {
          ...
          debugLog(
            'ACTIVATE_TEST_CLASSES',
            `Starting activation for test classes of ${containerClass}`,
            {
              session_id: session.session_id,
          ...
          if (activateResponse.isError) {
            throw new Error(
              `Activate test classes failed: ${activateResponse.content[0]?.text || 'Unknown error'}`,
            );
          }
          ...
          debugLog(
            'ACTIVATE_TEST_CLASSES',
            'Test classes activated successfully',
            {
          ...
          debugLog(
            'RUN_UNIT_TESTS',
            `Starting unit test run for ${containerClass}`,
            {
              session_id: session.session_id,
          ...
          if (runResponse.isError) {
            throw new Error(
              `Run unit tests failed: ${runResponse.content[0]?.text || 'Unknown error'}`,
            );
          }
          ...
          if (runId) {
            debugLog(
              'GET_TEST_STATUS',
              `Getting test run status for run_id: ${runId}`,
              {
                session_id: session.session_id,
                run_id: runId,
              },
          ...
          if (runId) {
            ...
            debugLog(
              'GET_TEST_RESULT',
              `Getting test run result for run_id: ${runId}`,
              {
                session_id: session.session_id,
                run_id: runId,
              },
          ...
          debugLog(
            'TEST_COMPLETE',
            `Full unit test workflow completed successfully for ${containerClass}`,
            {
              containerClass,
              testClass,
              runId,
          ...
          if (
            ...
            try {
              debugLog(
                'CLEANUP_ON_ERROR',
                `Attempting to unlock test classes ${containerClass} after error`,
                {
                  lock_handle: testClassesLockHandle,
                  session_id: testClassesLockSession.session_id,
                },
              ...
              debugLog(
                'CLEANUP_ON_ERROR',
                `Successfully unlocked test classes ${containerClass} after error`,
              );
            } catch (unlockError) {
              debugLog(
                'CLEANUP_ON_ERROR',
                `Failed to unlock test classes ${containerClass} after error`,
                {
                  unlockError:
              ...
              testLogger?.error(
                'Failed to unlock test classes after error:',
                unlockError,
              );
            }
          }
            ...
            try {
              ...
              if (
                ...
                try {
                  debugLog(
                    'CLEANUP',
                    `Attempting to unlock test classes ${containerClass} (cleanup)`,
                    {
                      lock_handle: testClassesLockHandle,
                      session_id: testClassesLockSession.session_id,
                    },
                  ...
                  debugLog(
                    'CLEANUP',
                    `Successfully unlocked test classes ${containerClass} (cleanup)`,
                  );
                  testLogger?.info(
                    `🔓 Unlocked test classes ${containerClass} (cleanup)`,
                  );
                } catch (unlockError: any) {
                  debugLog(
                    'CLEANUP',
                    `Failed to unlock test classes ${containerClass} (cleanup)`,
                    {
                      error:
                        unlockError instanceof Error
                          ? unlockError.message
                  ...
                  testLogger?.warn(
                    `⚠️  Failed to unlock test classes ${containerClass} during cleanup: ${unlockError.message || unlockError}`,
                  );
                }
              }
              ...
              debugLog(
                'CLEANUP',
                `Deletion excluded for diagnostics - object left for analysis: ${containerClass}`,
                {
                  container_class: containerClass,
                  test_class: testClass,
              ...
              testLogger?.info(
                `⚠️ Deletion excluded for diagnostics - object left for analysis: ${containerClass}`,
              );
            } catch (cleanupError) {
              debugLog(
                'CLEANUP_ERROR',
              ...
              testLogger?.warn(
                `⚠️  Failed to cleanup test class ${containerClass}: ${cleanupError}`,
              );
            }
          }
