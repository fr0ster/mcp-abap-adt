...
it.each(
  handlers,
)('%s stops at a refused schedule and never runs', async (_n, handler, args) => {
  const run = jest.fn();
  classExecutor = {
...
it.each(
  handlers,
)('%s reports a refused feed read rather than an empty feed', async (_n, handler, args) => {
  if (OPTION !== 'find-the-trace') return;
...
it.each(
  handlers,
)('%s reports a refusal during the search, not a missing trace', async (_n, handler, args) => {
  if (OPTION !== 'find-the-trace') return;
  let call = 0;
  classExecutor = {
    scheduleTrace: async () => okResponse(PROFILER_REQUEST),
...
it.each(
  handlers,
)('%s stops after max_trace_attempts and still reports the run', async (_n, handler, args) => {
  if (OPTION !== 'find-the-trace') return;
  let listed = 0;
  classExecutor = {
    scheduleTrace: async () => okResponse(PROFILER_REQUEST),
...
] as const)('%s answers its trace id where its own schema puts it', async (_n, handler, args, at) => {
  classExecutor = {
    scheduleTrace: async () => okResponse(PROFILER_REQUEST),
    runWithProfiler: async () => okResponse('done'),
  };
