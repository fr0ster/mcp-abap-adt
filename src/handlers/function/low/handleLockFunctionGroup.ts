/**
 * LockFunctionGroupLow Handler - Lock ABAP Function Group
 *
 * Uses AdtClient.getFunctionGroup().lock from @mcp-abap-adt/adt-clients 19.
 *
...
export async function handleLockFunctionGroup(
  ...
  return answer(
    { tool: 'LockFunctionGroupLow', detail: 'terse' },
    () =>
      createAdtClient(connection, logger)
        .getFunctionGroup()
        .lock({ functionGroupName }, { analyse: analyseLock }),
    (lockHandle: string) => ({
      success: true,
