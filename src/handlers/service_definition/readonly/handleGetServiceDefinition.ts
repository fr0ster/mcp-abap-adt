...
export const TOOL_DEFINITION = {
  ...
  description:
    '[read-only] Retrieve ABAP CDS Service Definition metadata and source code.',
  inputSchema: {
    service_definition_name: z
      .string()
      .describe('Name of the ABAP CDS Service Definition'),
  },
} as const;

function _parseServiceDefinitionXml(xml: string) {
  ...
  if (result['srvd:srvdSource'] || result.srvdSource) {
    ...
    return {
      ...
      masterLanguage:
        srvd['adtcore:masterLanguage'] || srvd.masterLanguage || null,
  ...
  try {
    if (!args?.service_definition_name) {
      throw new McpError(
        ...
        'Service definition name is required',
      );
    }
    const client = new ReadOnlyClient(connection);
    const response = await client.readServiceDefinition(
      args.service_definition_name,
    );

    if (!response) {
      throw new McpError(
        ErrorCode.InternalError,
        'Failed to read service definition',
      );
    }

    logger?.info(
      `Read service definition ${args.service_definition_name.toUpperCase()}`,
    );
    ...
    logger?.error(
      `Error reading service definition ${args?.service_definition_name || ''}: ${error instanceof Error ? error.message : String(error)}`,
    );
    return return_error(
      new Error(
        `Error reading service definition ${args?.service_definition_name || ''}: ${error instanceof Error ? error.message : String(error)}`,
