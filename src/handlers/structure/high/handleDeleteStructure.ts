/**
 * DeleteStructure Handler - Delete ABAP Structure via ADT deletion API
 *
...
export const TOOL_DEFINITION = {
  ...
  description:
    'Delete an ABAP structure from the SAP system via ADT deletion API. Transport request optional for $TMP objects.',
  inputSchema: {
    type: 'object',
    properties: {
      structure_name: {
...
export async function handleDeleteStructure(
  ...
  return answer(
    { tool: 'DeleteStructure', detail },
    () =>
      deleteIfDeletable(
        createAdtClient(connection, logger).getStructure(
