 ...
 * does forward `config.source` to the wire request — unlike `AdtTable`'s,
 ...
 * `config.source` for `check`.** See `UpdateStructureLow` for `update`.
 */
        ...
        (lockHandle): Promise<IAdtResponse<AdtReading<unknown>, IAdtError>> => {
          ...
          return shouldActivate
            ? sequence(
                () =>
                  obj.check(
                    { structureName, source: args.ddl_code },
                    'inactive',
                    { analyse: analyseException },
                  ),
                update,
