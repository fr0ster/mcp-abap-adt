...
export const TOOL_DEFINITION = {
  ...
  "inputSchema": {
    ...
    "properties": {
      ...
      "detailed": {
        "type": "boolean",
        "description": "[read-only] If true, returns all references including packages and internal components.",
        "default": false
      }
    },
    "required": [
 ...
 */
async function resolveObjectIdentifier(context: HandlerContext, objectIdentifier: string): Promise<string | null> {
 ...
 */
async function enhanceReferencesWithNames(context: HandlerContext, references: WhereUsedReference[]): Promise<WhereUsedReference[]> {
  const { connection, logger } = context;
  ...
  for (let i = 0; i < enhancedReferences.length; i++) {
    ...
    if (ref.objectIdentifier && (!ref.name || ref.name.length === 0)) {
      const resolvedName = await resolveObjectIdentifier(context, ref.objectIdentifier);
      if (resolvedName) {
