/**
 * Windows-specific file-based implementation of ServiceKeyStore for mcp-abap-adt
 *
 * Automatically resolves paths for Windows:
 * 1. Custom path (if provided)
 * 2. AUTH_BROKER_PATH environment variable
 * 3. %USERPROFILE%\Documents\mcp-abap-adt\service-keys (standard Windows location)
 * 4. Current working directory (process.cwd())
 *
 * Reads service keys from {destination}.json files in resolved paths.
 */

import { ServiceKeyStore, FileServiceKeyStore } from '@mcp-abap-adt/auth-broker';
import { getPlatformPaths } from './platformPaths';

/**
 * Windows-specific file-based service key store for mcp-abap-adt
 *
 * Automatically uses %USERPROFILE%\Documents\mcp-abap-adt\service-keys as default location
 ...
 */
export class WindowsFileServiceKeyStore implements ServiceKeyStore {
  private delegate: FileServiceKeyStore;

  /**
   * Create a new WindowsFileServiceKeyStore instance
   * @param customPath Optional custom path for service keys.
   *                   If not provided, uses %USERPROFILE%\Documents\mcp-abap-adt\service-keys
