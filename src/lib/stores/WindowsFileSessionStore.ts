/**
 * Windows-specific file-based implementation of SessionStore for mcp-abap-adt
 *
 * Automatically resolves paths for Windows:
 * 1. Custom path (if provided)
 * 2. AUTH_BROKER_PATH environment variable
 * 3. %USERPROFILE%\Documents\mcp-abap-adt\sessions (standard Windows location)
...
/**
 * Windows-specific file-based session store for mcp-abap-adt
 *
 * Automatically uses %USERPROFILE%\Documents\mcp-abap-adt\sessions as default location
 ...
 */
export class WindowsFileSessionStore implements ISessionStore {
  private delegate: FileSessionStore;

  /**
   * Create a new WindowsFileSessionStore instance
   * @param customPath Optional custom path for session files.
   *                   If not provided, uses %USERPROFILE%\Documents\mcp-abap-adt\sessions
