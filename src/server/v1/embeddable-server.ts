...
export class EmbeddableMcpServer {
  ...
  constructor(options: EmbeddableServerOptions) {
    ...
    this._mcpHandlers.RegisterAllToolsOnServer(this._mcpServer, context, this._exposition);

    context.logger?.info?.("EmbeddableMcpServer created with handlers registered", {
