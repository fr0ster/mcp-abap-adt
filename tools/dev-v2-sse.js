...
function ensureDistExists(entryPath) {
  if (!fs.existsSync(entryPath)) {
    process.stderr.write('[dev-v2-sse] dist/tools/test-v2-server-sse-compiled.js not found. Please run "npm run build" first.\n');
    process.exit(1);
  }
}
...
function spawnServer(entryPath, cliArgs) {
  ...
  child.on('error', (error) => {
    process.stderr.write(`[dev-v2-sse] ✗ Failed to spawn server: ${error.message}\n`);
    process.exit(1);
  });

  child.on('exit', (code, signal) => {
    const reason = code !== null ? `code ${code}` : `signal ${signal}`;
    process.stderr.write(`[dev-v2-sse] SSE server exited (${reason}).\n`);
    process.exit(code ?? 0);
  });
...
function spawnInspector(port) {
  ...
  child.on('error', (error) => {
    process.stderr.write(`[dev-v2-sse] ✗ Failed to spawn inspector: ${error.message}\n`);
  });

  child.on('exit', (code, signal) => {
    if (code !== null && code !== 0) {
      const reason = code !== null ? `code ${code}` : `signal ${signal}`;
      process.stderr.write(`[dev-v2-sse] MCP Inspector exited (${reason}).\n`);
    }
  });
...
(function main() {
  const entryPath = path.resolve(__dirname, './test-v2-server-sse-compiled.js');
  ensureDistExists(entryPath);

  const port = parseSsePort(args);
  const server = spawnServer(entryPath, args);

  const inspectorDelay = parseInt(process.env.MCP_DEV_V2_SSE_INSPECTOR_DELAY || '1200', 10);
  let inspector;
